...
class TeamRouter {
  ...
  static async getByUserId(ctx) {
    ...
    let team = await TeamModel.findOne({
      "managers.id": ctx.params.userId
    });

    if (!team) {
      team = await TeamModel.findOne({
        "confirmedUsers.id": ctx.params.userId
      });
    }
  ...
  static async confirmUser(ctx) {
    ...
    if (data) {
      ...
      if (team && !team.confirmedUsers.includes(email)) {
        ...
        team.confirmedUsers = team.confirmedUsers.concat({
          id: userId,
          email
        });
  ...
  static async delete(ctx) {
    ...
    const result = await TeamModel.remove({
      _id: ctx.params.id
    });
...
const isAuthenticatedMiddleware = async (ctx, next) => {
  ...
  const user = { ...(query.loggedUser ? JSON.parse(query.loggedUser) : {}),
    ...body.loggedUser
  };
